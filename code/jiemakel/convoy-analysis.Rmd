...
Original sample is defined as follows: <code>twarc2 search --archive --start-time '2022-01-22' --end-time '2022-02-05' 'lang:en ("#TruckersForFreedom" OR "#FreedomConvoy2022" OR "#TruckerConvoy2022" OR "#NoVaccineMandate" OR "#ConvoyForFreedom" OR "#FreedomConvoyCanada" OR "#FluTruxKlan" OR "#FluTrucksKlan" OR "#EndTheStigma" OR "#TrudeauIsACoward" OR "#CowardOfCanada" OR "#ResignTrudeau" OR "#Omicronvoy" OR "#OttawaOccupation" OR "#HonkHonk" OR "#honkhonk" OR "#TruckersForFreedom2020" OR "#freedomconvoy" OR "#OttawaOccupied" OR "#TrudeauMustResign" OR "#OttawaSiege" OR "#CanadaTruckers" OR "#OttawaOccupiers" OR "#freedomconvoy22" OR "#TrudeauTyranny" OR "#FluTruckKlan" OR "#GoHomeTruckers" OR "#ConvoyGoHome" OR "#GoHomeFluTruxclan" OR "#flutrucksclangohome" OR "#freedumbConvoy2022" OR "#Clownvoy" OR "#FluTruxKlanGoHome" OR "#GoHomeConvoy" OR "#IAmVaxxed" OR "#ConvoyGoHome" OR "Go Home" OR "Canadian Trucking Alliance" OR "PM Trudeau" OR "Prime Minister Justin Trudeau" OR "Prime Minister")' convoy_2weeks.jsonl</code>

## Raw access to the tweet data

The tweets are stored in a [MariaDB](https://mariadb.com/) (MySQL) database called `convoy` on host `vm1788.kaj.pouta.csc.fi`. The username for the database is `convoy`. The password is given separately. **DO NOT INCLUDE THE PASSWORD IN ANY CODE YOU COMMIT TO GITHUB**. If running this notebook, it will ask for the password the first time you run it, and then store it separately in your keyring.
 ...
 * The data has been loaded into both [Aria](kb/en/aria-storage-engine/) as well as [ColumnStore](https://mariadb.com/kb/en/mariadb-columnstore/) tables. Aria tables have a suffix of `_a` and ColumnStore tables a suffix of `_c`. 
 ...
 * The ColumnStore does not support [recursive queries](https://mariadb.com/kb/en/recursive-common-table-expressions-overview/) useful for thread analysis. Therefore, these have to be computed using Aria `_a` tables.
...
```{r}
tweets_c %>% 
  filter(original,date_created_at>=as.Date("2022-01-22"),date_created_at<as.Date("2022-02-05")) %>%
  group_by(date_created_at) %>%
  ...
  bind_rows(
    tweets_c %>% 
      ...
      filter(date_created_at>=as.Date("2022-01-22"),date_created_at<as.Date("2022-02-05")) %>%
      group_by(date_created_at) %>%
  ...
  bind_rows(
    tbl(con,sql('
      WITH RECURSIVE descendants AS ( 
        ...
        UNION 
        SELECT t.tweet_id
        FROM tweets_a t FORCE INDEX (in_reply_to), descendants d 
        WHERE 
          t.in_reply_to=d.tweet_id
      ) 
      SELECT date_created_at, COUNT(*) AS value
      FROM descendants INNER JOIN tweets_a USING (tweet_id)
      WHERE date_created_at>="2022-01-22" AND date_created_at<"2022-02-05"
      GROUP BY date_created_at
    ')) %>%
    collect() %>%
    mutate(name="total tweets in downstream discussions")
  ) %>%
  ggplot(aes(x=date_created_at,y=value,color=name)) +
  geom_step() +
  theme_hsci_discrete() +
...
```{r}
tweets_c %>%
  count(ur_conversation_id) %>%
  ggplot(aes(x=n)) +
  geom_histogram(bins=100) +
  scale_y_log10(labels=scales::comma) +
  scale_x_log10(labels=scales::comma) +
  theme_hsci_discrete() +
  ylab("Number of ur-conversations") +
  xlab("Tweets in ur-conversation")
...
ur_conversations_c %>% 
  arrange(desc(uc_ur_descendants)) %>% 
  head(10) %>%
  select(ur_conversation_id,created_at,uc_ur_descendants,ur_descendants,ur_mean_reply_count,ur_mean_retweet_count) %>%
  left_join(tweets_c %>% group_by(ur_conversation_id) %>% summarize(authors=n_distinct(author_id)),by=c("ur_conversation_id")) %>%
...
descendants <- function(tweet_id) { 
  tbl(con,sql(glue("
                   WITH RECURSIVE descendants AS ( 
                     SELECT t.tweet_id, 0 AS level 
                     FROM tweets_a t 
                     WHERE tweet_id={tweet_id} 
                     UNION 
                     SELECT t.tweet_id, d.level+1 AS level
                     FROM tweets_a t FORCE INDEX (in_reply_to), descendants d 
                     WHERE t.in_reply_to=d.tweet_id 
...
## Conversation 1254210334241419264 - the MOAC (Mother of All [Climate Change + Spam] Conversations)

```{r}
tweets_c_i %>% 
  filter(ur_conversation_id==sql("1254210334241419264")) %>%
  summarize(
    tweets=n(),
    tweets_with_keywords=sum(original),
  ...
  bind_cols(
    tweets_c_i %>% 
      filter(ur_conversation_id==sql("1254210334241419264")) %>%
      count(author_id,name="tweets_per_author") %>%
      mutate(m_t_p_a=mean(tweets_per_author)) %>%
      summarize(mean_tweets_per_author=mean(tweets_per_author),tweets_per_author_mean_absolute_deviation=mean(abs(m_t_p_a-tweets_per_author))) %>% collect()
  ) %>%
  gt()
```
...
tweets_c_i %>% 
  filter(ur_conversation_id==sql("1254210334241419264")) %>%
  left_join(users_c_i,by=c("author_id"="user_id")) %>%
  arrange(created_at.x) %>%
  head(20) %>% 
  mutate(tweet_id=str_c("[",tweet_id,"](https://twitter.com/a/status/",tweet_id,")")) %>%
  mutate(conversation_id=str_c("[",conversation_id,"](https://twitter.com/a/status/",conversation_id,")")) %>%
  mutate(ur_conversation_id=str_c("[",ur_conversation_id,"](https://twitter.com/a/status/",ur_conversation_id,")")) %>%
  gt() %>%
  fmt_markdown(tweet_id|conversation_id|ur_conversation_id)
```
...
tweets_c %>% 
  filter(ur_conversation_id==sql("1254210334241419264")) %>% 
  count(original,date_created_at) %>%
  collect() %>%
  mutate(original=if_else(original==1,"contains keywords","expanded conversation")) %>%
  ggplot(aes(x=date_created_at,y=n,fill=original)) +
...
tweets_c %>% 
  filter(ur_conversation_id==sql("1254210334241419264")) %>% 
  left_join(tweet_hashtags_c,by=c("tweet_id")) %>% 
  count(hashtag,year_created_at) %>% 
  group_by(year_created_at) %>% 
  slice_max(n,n=10) %>%
  gt(groupname_col="year_created_at",rowname_col="hashtag")
```


## Conversation 647567168167636992: the erotic fiction

```{r}
tweets_c_i %>% 
  filter(ur_conversation_id==sql("647567168167636992")) %>%
  summarize(
    tweets=n(),
    tweets_with_keywords=sum(original),
    authors=n_distinct(author_id),
...
## Conversation 1488162322187182085: Trudeau tests positive for COVID-19

```{r}
tweets_c_i %>% 
  filter(ur_conversation_id==sql("1488162322187182085")) %>%
  summarize(
    tweets=n(),
    tweets_with_keywords=sum(original),
    authors=n_distinct(author_id),
...
## Conversation 1225501300789477377: The British Slugfest

```{r}
tweets_c_i %>% 
  filter(ur_conversation_id==sql("1225501300789477377")) %>%
  summarize(
    tweets=n(),
    tweets_with_keywords=sum(original),
    authors=n_distinct(author_id),
...
## Conversation 1485881808747220996: random ur-conversation 1

```{r}
ur_conversation_id_key <- "1485881808747220996"
tweets_c_i %>% 
  filter(ur_conversation_id==sql(ur_conversation_id_key)) %>%
  summarize(
    tweets=n(),
    tweets_with_keywords=sum(original, na.rm=T),
    authors=n_distinct(author_id),
    min_date=min(created_at, na.rm=T),
    max_date=max(created_at, na.rm=T)
  ) %>% collect() %>%
  bind_cols(
    tweets_c_i %>% 
      filter(ur_conversation_id==sql(ur_conversation_id_key)) %>%
      count(author_id,name="tweets_per_author") %>%
      mutate(m_t_p_a=mean(tweets_per_author, na.rm=T)) %>%
      summarize(mean_tweets_per_author=mean(tweets_per_author),tweets_per_author_mean_absolute_deviation=mean(abs(m_t_p_a-tweets_per_author))) %>% collect()
  ) %>%
  gt()
```
...
## Conversation 1485881813692149764: random ur-conversation 2

```{r}
ur_conversation_id_key <- "1485881813692149764"
tweets_c_i %>% 
  filter(ur_conversation_id==sql(ur_conversation_id_key)) %>%
  summarize(
    tweets=n(),
...
## Conversation 1485881842322612226: random ur-conversation 3

```{r}
ur_conversation_id_key <- "1485881842322612226"
tweets_c_i %>% 
  filter(ur_conversation_id==sql(ur_conversation_id_key)) %>%
  summarize(
    tweets=n(),
...
## Conversation 1486352737935798274: random "real" (10-100 message) conversation 1

```{r}
conversation_id_key <- "1486352737935798274"
tweets_c_i %>% 
...
## Conversation 1486352747347812353: random "real" (10-100 message) conversation 2

```{r}
conversation_id_key <- "1486352747347812353"
tweets_c_i %>% 
  filter(conversation_id==sql(conversation_id_key)) %>%
  summarize(
    tweets=n(),
...
## Conversation 1486352996304830466: random "real" (10-100 message) conversation 3

```{r}
conversation_id_key <- "1486352996304830466"
tweets_c_i %>% 
  filter(conversation_id==sql(conversation_id_key)) %>%
  summarize(
    tweets=n(),
...
## Conversation 1485662249624780807: random "real" "long" (50-150 message, max_depth>=20, authors<messages/3, keywords in at least messages/10, starting >=2022-01-22) conversation 1

Code for pulling random "real" conversation ids:
```
conversations_c_i %>% 
 filter(created_at>=as.Date("2022-01-22"),c_descendants>=50,c_descendants<=200,max_depth>=20,!is.na(tweet_id),authors<c_descendants/3) %>%
 left_join(tweets_c_i %>% 
   filter(original) %>% 
   count(conversation_id,name="originals") ,by=c("conversation_id")
  ) %>% 
  filter(originals>=c_descendants/10) %>% 
  arrange(sql("RAND()")) %>% 
  head(3) %>% 
  pull(conversation_id)
 ``` output: `1486700611928309763 1486730979968495616 1486100674152906752`

```{r}
conversation_id_key <- "1486700611928309763"
tweets_c_i %>% 
  filter(conversation_id==sql(conversation_id_key)) %>%
  summarize(
    tweets=n(),
...
## Conversation 1486730979968495616: random "real" "long" (50-150 message, max_depth>=20, authors<messages/3, keywords in at least messages/10, starting >=2022-01-22) conversation 2

```{r}
conversation_id_key <- "1486730979968495616"
tweets_c_i %>% 
  filter(conversation_id==sql(conversation_id_key)) %>%
  summarize(
    tweets=n(),
...
## Conversation 1486100674152906752: random "real" "long" (50-150 message, max_depth>=20, authors<messages/3, keywords in at least messages/10, starting >=2022-01-22) conversation 3

```{r}
conversation_id_key <- "1486100674152906752"
tweets_c_i %>% 
  filter(conversation_id==sql(conversation_id_key)) %>%
  summarize(
    tweets=n(),
